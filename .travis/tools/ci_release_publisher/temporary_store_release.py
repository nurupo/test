...
def cleanup(releases, scopes, release_completenesses, on_nonallowed_failure, github_api_url, travis_api_url):
    ...
    if on_nonallowed_failure:
        # Jobs are marked as failed only once they complete, so if we want to check if the current job has failed,
        # we have to check the env variables instead of Travis-CI API, as the API won't tell us this.
